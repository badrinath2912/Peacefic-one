  ...
  if (result) {
    return (
      <div className="mx-auto w-full max-w-md space-y-6">
        ...
        {/* Sign-in is refused until the address is verified, so the code is the
            actual next step and gets the primary action. */}
        <Button asChild block>
          <Link href={`/verify-email?email=${encodeURIComponent(result.email)}`}>
            Verify my email
          </Link>
        </Button>
